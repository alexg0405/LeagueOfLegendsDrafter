...
Signing rewrites the installer, so this script also regenerates the blockmap and refreshes the
hash/size in `latest.yml`. Re-run `npm run release:stage-web` afterwards so the website serves the
signed artifacts and a matching update feed.

## Verify

```powershell
Get-AuthenticodeSignature .\release\Nexus-Draft-Setup-4.3.0.exe
Get-AuthenticodeSignature .\release\Nexus-Draft-Portable-4.3.0.exe
```
